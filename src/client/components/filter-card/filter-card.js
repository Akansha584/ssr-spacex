...
const getLaunchYear = (handleClick, filters) => {
    let years = YEARS.map((year) => {
        let filteredColor = filters["launch_year"] === year ? "bck-click-gr" : "bck-gr";
        return (
            <button
                className={`mr_31 mb_24 p-button br5 ${filteredColor}`}
        ...
        <div className="mb_12">
            ...
            <div className="fr spb mt_12">
                <button
                    ...
                    onClick={() => handleClick(true, key)}>True</button>
                <button
                    className={`p-button br5 ${filteredColorForFalse}`}
                    onClick={() => handleClick(false, key)}>False</button>
            </div>
        </div>
    )
}
    ...
    return (
        <div className="card w200 mr_30 pt_8" style={{ height: 'fit-content' }}>
            ...
            <div className="frc alIc">
                ...
                {getLaunchAndLandFilters(SUCCESSFUL_LAUNCH, handleClick, "launch_success", filters)}
                {getLaunchAndLandFilters(SUCCESSFUL_LANDING, handleClick, "land_success", filters)}
            </div>
        </div>
    )
}
